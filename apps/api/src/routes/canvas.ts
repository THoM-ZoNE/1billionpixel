...
import { getCanvasAreas, getCanvasStats, claimArea } from "../services/canvas.js";
...
function mimeToExt(mime: string): "webp" | "gif" | "jpg" | "png" {
  ...
  return "jpg"; // image/jpeg és minden más → jpg
}
    ...
    async (req, reply) => {
      ...
      for await (const part of parts) {
        if (part.type === "file" && part.fieldname === "image") {
          ...
          // ✅ mimeToExt() konverzió → TypeScript hiba megszűnik
          const saved = await saveImageLocally(buffer, mimeToExt(mime));
          imageUrl = saved.url;
          imageKey = saved.key;
          imageType = mimeToExt(mime);
      ...
      if (
        ...
        width <= 0 || height <= 0 ||
        x < 0 || y < 0 ||
        x + width > 100000 || y + height > 100000
      ) {
      ...
      try {
        ...
        // ✅ Telegram handle mentése (ha a ClaimModal elküldte)
        const telegramHandle = fields.telegramHandle?.trim();
        if (telegramHandle) {
          await prisma.wallet.update({
            where: { address: walletAddress },
            data: {
              telegramHandle: telegramHandle.startsWith("@")
                ? telegramHandle
                : `@${telegramHandle}`,
            },
          });
        ...
        app.log.error({ err }, "[claim] Internal error");
